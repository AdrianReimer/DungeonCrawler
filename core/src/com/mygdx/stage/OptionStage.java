...
public class OptionStage extends Stage {
     ...
     */
	public OptionStage (final StageSwitchInterface stageSwitchInterface, SoundManager soundManager) {
		...
		// add Windows
        Window displaySettingsWindow = new Window(GameTexts.OPTION_STAGE_DISPLAY_SETTINGS_TEXT.get(), skin, GameConstants.DIALOG_WINDOW_STYLE);
        displaySettingsWindow.pad(WINDOW_PAD);
        Window gameSettingsWindow = new Window(GameTexts.OPTION_STAGE_GAME_SETTINGS_TEXT.get(), skin, GameConstants.DIALOG_WINDOW_STYLE);
        gameSettingsWindow.pad(WINDOW_PAD);
        gameSettingsWindow.setMovable(false);
        Window soundSettingsWindow = new Window(GameTexts.OPTION_STAGE_SOUND_SETTINGS_TEXT.get(), skin, GameConstants.DIALOG_WINDOW_STYLE);
        soundSettingsWindow.pad(WINDOW_PAD);
        soundSettingsWindow.setMovable(false);
        Window musicSettingsWindow = new Window(GameTexts.OPTION_STAGE_MUSIC_SETTINGS_TEXT.get(), skin, GameConstants.DIALOG_WINDOW_STYLE);
        ...
        // add Sliders
        final Slider soundEffectMasterSlider = new Slider(0, 10, 0.1f, false, skin);
        soundEffectMasterSlider.setValue(soundManager.getSoundEffect().getSoundEffectMaster());
        soundEffectMasterSlider.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeListener.ChangeEvent event, Actor actor) {
            	soundManager.getSoundEffect().setSoundEffectMaster(soundEffectMasterSlider.getValue());
            }
        });
        final Slider soundScapeMasterSlider = new Slider(0, 10, 0.1f, false, skin);
        soundScapeMasterSlider.setValue(soundManager.getSoundscape().getSoundScapeMaster());
